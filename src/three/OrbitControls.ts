...
export class OrbitControls extends THREE.EventDispatcher {
    ...
    constructor(object: THREE.Camera, domElement?: HTMLElement, domWindow?: Window) {
        ...
        this.onMouseWheel = (event: ThreeEvent) => {
            ...
            if (this.enabled === false || this.enableZoom === false || ( this.state !== STATE.NONE && this.state
                                                                                                      !== STATE.ROTATE )) {
                return;
            }
    ...
    update() {
        ...
        (this.spherical as (any) as any).theta =
            Math.max(this.minAzimuthAngle, Math.min(this.maxAzimuthAngle, (this.spherical as any).theta));

        // restrict phi to be between desired limits
        (this.spherical as any).phi =
            Math.max(this.minPolarAngle, Math.min(this.maxPolarAngle, (this.spherical as any).phi));
        ...
        (this.spherical as any).radius =
            Math.max(this.minDistance, Math.min(this.maxDistance, (this.spherical as any).radius));
